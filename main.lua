...
function QuickApp:onInit()
    ...
    self:trace('')
    self:trace('Netatmo noise sensor')
    self:trace('User:', self.config:getUsername())
    self:updateProperty('manufacturer', 'Netatmo')
    self:updateProperty('manufacturer', 'Noise sensor')
    self:updateView("label2", "text", string.format(self.i18n:get('Noise level'), 0))
    self:run()
end
...
function QuickApp:pullNetatmoData()
    ...
    local callback = function(response)
        ...
        if device ~= nil then
            self:updateProperty("value", device.dashboard_data.Noise)
            self:updateProperty("unit", "dB")
            self:updateView("label2", "text", string.format(self.i18n:get('Noise level'), device.dashboard_data.Noise))
            self:trace('[' .. device['_id'] .. '] Noise information updated')
